...
<template>
    <div class="audio-container">
        <audio
            autoplay
            ref="audio"
            :src="musicList[playState.index]?.url"
            @play="playState.play = true"
        ></audio>
        <!-- 封面 -->
        <div class="cover">
            <div
                ...
                @click="audioPlay"
                :title="
                    musicList[playState.index]
                        ? musicList[playState.index].name
                        : '暂无封面'
                "
            >
        ...
        <div
            class="audio-main"
            @mousemove.prevent="volumeClickMove($event)"
            @mouseup="volumeMoveFlag = false"
            ...
            <div class="control">
                ...
                <i
                    :class="[
                        'iconfont',
                        'play',
                        playState.play ? 'icon-zanting' : 'icon-bofang',
                    ]"
                    @click="audioPlay"
                ></i>
                ...
                <div class="volume-box">
                    <div
                        ref="volumeProgressBarDom"
                        class="volume-progress"
                        :style="{ display: volumeMoveFlag ? 'block' : '' }"
                        @mousedown="volumeClickDown($event)"
                    >
                ...
                <i
                    :class="[
                        'iconfont',
                        'play-order',
                        playState.playOrder === 'along'
                            ? 'icon-xunhuanbofang'
                            : playState.playOrder === 'random'
                            ? 'icon-suijibofang'
                            : 'icon-danquxunhuan',
                    ]"
                ...
                <i
                    :class="[
                        'iconfont',
                        'lyrics',
                        playState.lyrics ? 'icon-gecikai' : 'icon-geciguan',
                    ]"
                    @click="playState.lyrics = !playState.lyrics"
                ></i>
                <!-- 歌单列表 -->
                <i
                    class="iconfont icon-liebiao"
                    @click="playState.playList = !playState.playList"
                ></i>
            </div>
            <!-- 歌曲进度条 -->
            <div
                ...
                :class="[
                    'audio-progress',
                    ProgressDomLock ? 'wait' : 'pointer',
                    playState.buffer || ProgressMoveFlag
                        ? 'audio-progress-hover'
                        : '',
                ]"
                @mousedown="
                    () => {
                        if (playState.getDuration) ProgressMoveFlag = true;
                    }
                "
            >
                <!-- 缓冲 -->
                <div
                    ...
                    :style="{
                        width:
                            ((cache.end - cache.start) / playState.duration) *
                                100 +
                            '%',
                ...
                <div
                    class="elaps"
                    progress="true"
                    :style="{ width: playState.progress * 100 + '%' }"
                >
                    <div class="thumb" progress="true">
                        <i
                            progress="true"
                            v-show="playState.buffer"
                            class="iconfont icon-buffer"
                        ></i>
                    </div>
                </div>
            </div>
        ...
        <div
            ref="musicListDom"
            :class="['music-list', playState.playList ? 'show-list' : '']"
        >
            <div
    ...
    if (audio.value) {
        ...
        audio.value.addEventListener("timeupdate", () => {
            ...
            if (playState.lyrics) {
                ...
                let index =
                    playState.lrcList.findIndex(
                        (lrc) => lrc.lineTime > audio.value.currentTime
                    ) - 1;
...
watch(
    ...
    (newVal) => {
        ...
        if (playState.playOrder !== "random")
            randomIndex.value = randomIndexList.findIndex(
                (item) => item === newVal
            );
...
const volumeClickMove = (event: MouseEvent) => {
    if (volumeMoveFlag.value) {
        ...
        let abr =
            ((progressBarHeight + progressBarOffsetTop - event.y) /
                progressBarHeight) *
            100;
        abr = abr < 0 ? 0 : abr;
        abr = abr > 100 ? 100 : abr;
...
const changeProgress = (event: MouseEvent) => {
    ...
    if (
        ProgressMoveFlag.value ||
        (event.target as HTMLElement).getAttribute("progress")
    ) {
        // 总进度条 X 坐标
